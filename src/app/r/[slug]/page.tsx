...
function CustomerFlow({
  ...
  // When user selects review and taps "Continue to Post on Google"
  const handlePostToGoogle = async (reviewText: string) => {
    setFinalReviewText(reviewText);

    // 1. Record review in business dashboard INSTANTLY
    if (campaign) {
      try {
          ...
          {step === "google-redirect" && (
            <div className="flex-1 flex flex-col px-6 pt-8 pb-5">
              <div className="flex flex-col items-center mb-8">
                ...
                <h2 className="text-[20px] font-extrabold text-[#1A1A2E] mb-1">Review Copied!</h2>
                <p className="text-[13px] text-[#6B7280] text-center">Your review is on your clipboard and ready to paste</p>
              </div>
              ...
              <div className="space-y-2.5 mb-6">
                {[
                  { num: "1", text: "Tap the button below to open Google", done: false },
                  { num: "2", text: "Long press to paste your review", done: false },
                  { num: "3", text: "Submit and come back for your reward!", done: false },
                ].map((s, i) => (
              ...
              <div className="mt-auto space-y-3">
                ...
                <button onClick={() => { try { navigator.clipboard.writeText(finalReviewText); } catch {} }}
                  className="w-full flex items-center justify-center gap-2 py-3 rounded-2xl border border-[#E5E7EB] text-[13px] text-[#6B7280] font-semibold hover:bg-[#F9FAFB] transition-colors">
                  <Copy className="w-4 h-4" /> Copy Review Again
                </button>
              </div>
            </div>
          )}
