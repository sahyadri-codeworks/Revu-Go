    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-[#F3F4F6] p-6">
            ...
            <div key={month} className="space-y-1.5">
              ...
              <div className="flex gap-1.5">
                <div className="flex-1 h-3 bg-[#F3F4F6] rounded-full overflow-hidden">
                  <div
                    className="h-full bg-gradient-to-r from-[#166534] to-[#22C55E] rounded-full transition-all"
                    style={{ width: `${(values.businesses / maxBiz) * 100}%` }}
                  />
                </div>
                <div className="flex-1 h-3 bg-[#F3F4F6] rounded-full overflow-hidden">
                  <div
                    className="h-full bg-gradient-to-r from-[#D4AF37] to-[#FBBF24] rounded-full transition-all"
                    style={{ width: `${(values.reviews / maxReviews) * 100}%` }}
                  />
                </div>
              </div>
            </div>
        ...
        <div className="flex items-center gap-6 mt-4 pt-3 border-t border-[#F3F4F6]">
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-gradient-to-r from-[#166534] to-[#22C55E]" />
            <span className="text-[10px] text-[#6B7280]">Businesses</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-gradient-to-r from-[#D4AF37] to-[#FBBF24]" />
      ...
      <div className="bg-white rounded-2xl border border-[#F3F4F6] p-6">
        <div className="flex items-center gap-2 mb-5">
          ...
          <h3 className="text-[14px] font-bold text-[#111]">City Distribution</h3>
        </div>
        {cities.length === 0 ? (
            ...
            {cities.slice(0, 10).map(([city, count]) => (
              <div key={city} className="flex items-center gap-3">
                ...
                <div className="flex-1 h-5 bg-[#F3F4F6] rounded-full overflow-hidden">
                  <div
                    className="h-full bg-gradient-to-r from-[#166534] to-[#4ADE80] rounded-full flex items-center justify-end pr-2 transition-all"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-white rounded-2xl border border-[#F3F4F6] p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-[14px] font-bold text-[#111]">Conversion Metrics</h3>
          </div>
          <div className="space-y-3">
            <div className="flex items-center justify-between py-2 border-b border-[#F9FAFB]">
              <span className="text-[13px] text-[#6B7280]">Coupon Redemption Rate</span>
              <span className="text-[13px] font-bold text-[#111]">
                {data.totalCoupons > 0 ? Math.round((data.redeemedCoupons / data.totalCoupons) * 100) : 0}%
              </span>
            </div>
            <div className="flex items-center justify-between py-2 border-b border-[#F9FAFB]">
              <span className="text-[13px] text-[#6B7280]">Campaign Activity Rate</span>
              <span className="text-[13px] font-bold text-[#111]">
                {data.totalCampaigns > 0 ? Math.round((data.activeCampaigns / data.totalCampaigns) * 100) : 0}%
              </span>
            </div>
            <div className="flex items-center justify-between py-2">
              <span className="text-[13px] text-[#6B7280]">Reviews per Business</span>
              <span className="text-[13px] font-bold text-[#111]">
        ...
        <div className="bg-white rounded-2xl border border-[#F3F4F6] p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-[14px] font-bold text-[#111]">Platform Quality</h3>
          </div>
          <div className="space-y-3">
            <div className="flex items-center justify-between py-2 border-b border-[#F9FAFB]">
              <span className="text-[13px] text-[#6B7280]">Average Star Rating</span>
              <div className="flex items-center gap-1">
                <Star className="w-3.5 h-3.5 text-[#D4AF37] fill-[#D4AF37]" />
                <span className="text-[13px] font-bold text-[#111]">{data.avgRating}</span>
              </div>
            </div>
            <div className="flex items-center justify-between py-2 border-b border-[#F9FAFB]">
              <span className="text-[13px] text-[#6B7280]">Private Feedback Received</span>
              <span className="text-[13px] font-bold text-[#111]">{data.totalFeedback}</span>
            </div>
            <div className="flex items-center justify-between py-2">
              <span className="text-[13px] text-[#6B7280]">Business Active Rate</span>
