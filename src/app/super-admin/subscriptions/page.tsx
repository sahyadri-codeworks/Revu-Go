      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
        <div className="relative flex-1 w-full sm:max-w-sm">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-[#E5E7EB] bg-white text-sm text-[#111] placeholder:text-[#9CA3AF] focus:outline-none focus:border-[#166534] focus:ring-2 focus:ring-[#166534]/10"
          />
        </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-[#F3F4F6]">
                    <th className="text-left px-5 py-3 text-[10px] text-[#9CA3AF] font-semibold uppercase tracking-wider">Business</th>
                    <th className="text-left px-5 py-3 text-[10px] text-[#9CA3AF] font-semibold uppercase tracking-wider">Plan</th>
                    <th className="text-left px-5 py-3 text-[10px] text-[#9CA3AF] font-semibold uppercase tracking-wider">Status</th>
                    <th className="text-left px-5 py-3 text-[10px] text-[#9CA3AF] font-semibold uppercase tracking-wider">Start Date</th>
                    <th className="text-left px-5 py-3 text-[10px] text-[#9CA3AF] font-semibold uppercase tracking-wider">End Date</th>
                    <th className="text-left px-5 py-3 text-[10px] text-[#9CA3AF] font-semibold uppercase tracking-wider">Validity</th>
                    <th className="text-right px-5 py-3 text-[10px] text-[#9CA3AF] font-semibold uppercase tracking-wider">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {filtered.map((s) => {
                    ...
                    return (
                      <tr key={s.id} className="border-b border-[#F9FAFB] hover:bg-[#FAFBFC]">
                        ...
                        <td className="px-5 py-3.5">
                          <span className="px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider" style={{ backgroundColor: sc.bg, color: sc.text }}>
                            {s.status}
                          </span>
                        </td>
                        <td className="px-5 py-3.5 text-[12px] text-[#6B7280]">
                          {s.current_period_start ? new Date(s.current_period_start).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" }) : "—"}
                        </td>
                        <td className="px-5 py-3.5 text-[12px] text-[#6B7280]">
                          ...
                          {(() => {
                            ...
                            return (
                              <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                                daysLeft <= 0 ? "bg-[#FEF2F2] text-[#EF4444]" : daysLeft <= 7 ? "bg-[#FFFBEB] text-[#D97706]" : "bg-[#F0FDF4] text-[#166534]"
                              }`}>
                ...
                return (
                  <div key={s.id} className="p-4">
                      ...
                      {(() => {
                        ...
                        return (
                          <span className={`px-1.5 py-0.5 rounded text-[9px] font-bold ${
                            daysLeft <= 0 ? "bg-[#FEF2F2] text-[#EF4444]" : daysLeft <= 7 ? "bg-[#FFFBEB] text-[#D97706]" : "text-[#166534]"
                          }`}>
                            {daysLeft <= 0 ? "Expired" : `${daysLeft}d left`}
                          </span>
                        );
                    ...
                    <div className="flex items-center justify-between text-[10px] text-[#9CA3AF] mb-2">
                      <span>Start: {s.current_period_start ? new Date(s.current_period_start).toLocaleDateString("en-IN", { day: "2-digit", month: "short" }) : "—"}</span>
                      <span>End: {s.current_period_end ? new Date(s.current_period_end).toLocaleDateString("en-IN", { day: "2-digit", month: "short" }) : "—"}</span>
                    </div>
                    <button onClick={() => openEdit(s)} className="w-full py-2 rounded-lg border border-[#E5E7EB] text-[11px] font-medium text-[#6B7280]">
                      Edit Subscription
                    </button>
                  </div>
                );
        ...
        <div className="fixed inset-0 z-50 bg-black/30 flex items-center justify-center p-4" onClick={() => setEditModal(null)}>
          <div className="bg-white rounded-2xl w-full max-w-sm p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => setEditModal(null)} className="w-8 h-8 rounded-lg hover:bg-[#F3F4F6] flex items-center justify-center">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="space-y-3">
              <div>
                ...
                <select value={editStatus} onChange={(e) => setEditStatus(e.target.value)}
                  className="w-full px-3 py-2.5 rounded-lg border border-[#E5E7EB] text-sm text-[#111] focus:outline-none focus:border-[#166534]">
                  {["trial", "active", "cancelled", "expired", "suspended"].map((s) => (
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[10px] text-[#6B7280] uppercase tracking-wider font-medium mb-1 block">Start Date</label>
                  <input type="date" value={editStartDate} onChange={(e) => setEditStartDate(e.target.value)}
                    className="w-full px-3 py-2.5 rounded-lg border border-[#E5E7EB] text-sm text-[#111] focus:outline-none focus:border-[#166534]" />
                </div>
                <div>
                  <label className="text-[10px] text-[#6B7280] uppercase tracking-wider font-medium mb-1 block">End Date</label>
                  <input type="date" value={editEndDate} onChange={(e) => setEditEndDate(e.target.value)}
                    className="w-full px-3 py-2.5 rounded-lg border border-[#E5E7EB] text-sm text-[#111] focus:outline-none focus:border-[#166534]" />
                </div>
              </div>
              <div>
                <label className="text-[10px] text-[#6B7280] uppercase tracking-wider font-medium mb-1 block">Trial End Date</label>
