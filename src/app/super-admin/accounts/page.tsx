...
export default function AccountsPage() {
  ...
  const inputClass = "w-full px-3.5 py-2.5 rounded-xl border border-[#E5E7EB] text-sm text-[#111] placeholder:text-[#9CA3AF] focus:outline-none focus:border-[#166534] focus:ring-2 focus:ring-[#166534]/10 transition-all";
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Action menu dropdown (fixed position, rendered outside table) */}
      {actionMenu && (() => {
            ...
            <div className="p-6">
              ...
              <div className="space-y-2">
                {[
                  ...
                  ["Subscription", selectedAccount.subscription?.status ? selectedAccount.subscription.status.charAt(0).toUpperCase() + selectedAccount.subscription.status.slice(1) : "None"],
                  ["Valid Until", (() => {
                    const endDate = selectedAccount.subscription?.current_period_end || selectedAccount.subscription?.trial_ends_at;
                    if (!endDate) return "—";
                    const end = new Date(endDate);
                    const daysLeft = Math.ceil((end.getTime() - Date.now()) / (1000 * 60 * 60 * 24));
                    const dateStr = end.toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" });
                    return daysLeft <= 0 ? `Expired (${dateStr})` : `${dateStr} (${daysLeft}d left)`;
                  })()],
