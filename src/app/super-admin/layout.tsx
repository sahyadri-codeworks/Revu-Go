...
export default function SuperAdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const pageName = navItems.find((n) => n.href === pathname)?.label || "Super Admin";
    ...
    <div className="min-h-screen bg-[#F8FAFB]">
      ...
      <aside className="hidden lg:flex flex-col w-[260px] bg-white h-screen fixed left-0 top-0 z-40 border-r border-[#E5E7EB]">
        <div className="px-5 pt-6 pb-5">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[10px] text-[#166534] font-semibold uppercase tracking-wider ml-1">Admin</span>
          </div>
        </div>
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-white border-b border-[#E5E7EB] h-14 flex items-center justify-between px-4">
        <div className="flex items-center gap-2.5">
          <img src="/logo-name.png" alt="RevuGo" className="h-9 object-contain mix-blend-multiply" />
          <span className="font-bold text-sm text-[#111]">Super Admin</span>
        </div>
